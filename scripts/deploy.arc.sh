#!/usr/bin/env bash
# Deploy StakeAndAdvance to Arc testnet (reads .env). Sets the World ID backend signer
# when WORLD_ID_SIGNER is present. Uses the project-local Foundry toolchain.
set -euo pipefail
...
echo "[deploy] worldIdSigner: ${WORLD_ID_SIGNER:-<unset>}"
